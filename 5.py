...
def plot_result(func, pop_size, xs, x_hist, fs, best, i, m, num_var, low, high, time):
    if num_var == 1:
    ...
    x = np.arange(np.size(best)) * pop_size
...
def plot_result_1(func, num_sim, xs, x_hist, fitness, i, num_var,  low, high, time):
    ...
    plt.figure(figsize=(14, 6))
    x = np.arange(np.size(fitness))*num_sim
...
end1 = time.time()
plot_result(func,pop_size, xs, x_hist, fs, best, i, m, num_var = num_var, low=low, high=high, time = end1 - start1)
...
end2 = time.time()
plot_result_1(func, num_sim, xs, x_hist, fitness, i, num_var = num_var, low=low, high=high, time = end2 - start2)
